...
impl SubmissionModel {
    ...
    ) -> Result<SubmissionModel, sqlx::Error> {
    ...
    pub async fn find_by_id(
        pool: &PgPool,
        id: Uuid,
    ) -> Result<Option<SubmissionModel>, sqlx::Error> {
        let record =
            sqlx::query_as::<_, SubmissionModel>("SELECT * FROM submissions WHERE id = $1")
                .bind(id)
                .fetch_optional(pool)
                .await?;
    ...
    pub async fn find_by_bounty(
        pool: &PgPool,
        bounty_id: Uuid,
    ) -> Result<Vec<SubmissionModel>, sqlx::Error> {
    ...
    pub async fn update_status(pool: &PgPool, id: Uuid, status: &str) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE submissions SET status = $1, processed_at = $2 WHERE id = $3")
            .bind(status)
            .bind(Utc::now())
            .bind(id)
            .execute(pool)
    ...
    pub async fn update_accuracy_score(
        pool: &PgPool,
        id: Uuid,
        score: f32,
    ) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE submissions SET accuracy_score = $1 WHERE id = $2")
            .bind(score)
            .bind(id)
            .execute(pool)
            .await?;
    ...
    pub async fn count_by_bounty(pool: &PgPool, bounty_id: Uuid) -> Result<i64, sqlx::Error> {
        let result: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM submissions WHERE bounty_id = $1")
                .bind(bounty_id)
                .fetch_one(pool)
                .await?;
