...
use crate::models::{bounty::BountyModel, payout::PayoutModel, submission::SubmissionModel};
...
impl BlockchainSyncService {
            ...
            .filter_map(|log| {
                ...
                let data = match event_type {
                    ...
                    BlockchainEventType::PayoutDistributed => {
                        ...
                        let raw_hex: String =
                            log.data.iter().map(|b| format!("{:02x}", b)).collect();
                        serde_json::json!({
                            "bounty_id": bounty_id,
    ...
    async fn get_current_block_number(&self) -> Result<u64, SyncError> {
        ...
        let block_number = client.get_block_number().await.map_err(|e| {
            SyncError::BlockchainError(format!("Failed to get block number: {}", e))
        })?;
