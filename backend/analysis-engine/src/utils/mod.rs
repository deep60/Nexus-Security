...
pub use file_handler::{AnalysisStatus, FileHandler, FileMetadata as UtilsFileMetadata};
...
pub mod constants {
    ...
    pub const SUPPORTED_EXTENSIONS: &[&str] = &[
        "exe", "dll", "bat", "cmd", "scr", "pif", "com", "vbs", "js", "jar", "zip", "rar", "7z",
        "tar", "gz", "pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "rtf", "apk", "ipa",
        "deb", "rpm", "msi", "dmg", "bin",
    ];
    ...
    pub fn calculate_confidence(
        ...
        for (engine, is_malicious) in verdicts {
            let stake = stakes
                .get(engine)
                .cloned()
                .unwrap_or(constants::DEFAULT_STAKE_AMOUNT);
            total_stake += stake;
...
pub mod macros {
    ...
    macro_rules! require_field {
        ($field:expr, $field_name:expr) => {
            $field.ok_or_else(|| {
                EngineError::ValidationError(format!("Required field '{}' is missing", $field_name))
            })?
        };
    }
}
